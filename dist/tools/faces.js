...
export function registerFaceTools(server) {
    server.registerTool("dual_list_faces", {
        ...
        try {
            const res = await makeApiRequest("faces", "GET", undefined, params);
    ...
    server.registerTool("dual_create_face", {
        ...
        try {
            const res = await makeApiRequest("faces", "POST", params);
    ...
    server.registerTool("dual_get_face", {
        ...
        try {
            const res = await makeApiRequest(`faces/${params.face_id}`);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_update_face", {
        ...
        try {
            ...
            const res = await makeApiRequest(`faces/${face_id}`, "PATCH", body);
    ...
    server.registerTool("dual_get_template_faces", {
        ...
        try {
            const res = await makeApiRequest(`faces/template/${params.template_id}`);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
