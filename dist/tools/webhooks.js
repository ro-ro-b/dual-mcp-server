...
export function registerWebhookTools(server) {
    server.registerTool("dual_list_webhooks", {
        ...
        try {
            const res = await makeApiRequest("webhooks", "GET", undefined, params);
    ...
    server.registerTool("dual_create_webhook", {
        ...
        try {
            const res = await makeApiRequest("webhooks", "POST", params);
    ...
    server.registerTool("dual_get_webhook", {
        ...
        try {
            const res = await makeApiRequest(`webhooks/${params.webhook_id}`);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_update_webhook", {
        ...
        try {
            ...
            const res = await makeApiRequest(`webhooks/${webhook_id}`, "PATCH", body);
    ...
    server.registerTool("dual_test_webhook", {
        ...
        try {
            const { webhook_id, ...body } = params;
            const res = await makeApiRequest(`webhooks/${webhook_id}/test`, "POST", body);
