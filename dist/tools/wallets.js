...
export function registerWalletTools(server) {
    ...
    server.registerTool("dual_login", {
        ...
        try {
            const res = await makeApiRequest("wallets/login", "POST", params);
    ...
    server.registerTool("dual_login_guest", {
        ...
        try {
            const res = await makeApiRequest("wallets/login/guest", "POST", params);
            setAuth(res.access_token, res.refresh_token);
    ...
    server.registerTool("dual_register_verify", {
        ...
        try {
            const res = await makeApiRequest("wallets/register/verify", "POST", params);
            setAuth(res.access_token, res.refresh_token);
    ...
    server.registerTool("dual_refresh_token", {
        ...
        try {
            const res = await makeApiRequest("wallets/token/refresh", "POST", params);
            setAuth(res.access_token, res.refresh_token);
    ...
    server.registerTool("dual_get_me", {
        ...
        try {
            const w = await makeApiRequest("wallets/me");
    ...
    server.registerTool("dual_update_me", {
        ...
        try {
            const w = await makeApiRequest("wallets/me", "PATCH", params);
    ...
    server.registerTool("dual_get_wallet", {
        ...
        try {
            const w = await makeApiRequest(`wallets/${params.wallet_id}`);
            return textResult(JSON.stringify(w, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
