...
export function registerNotificationTools(server) {
    server.registerTool("dual_list_messages", {
        ...
        try {
            const res = await makeApiRequest("messages", "GET", undefined, params);
    ...
    server.registerTool("dual_list_message_templates", {
        ...
        try {
            const res = await makeApiRequest("messages/templates");
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_create_message_template", {
        ...
        inputSchema: {
            ...
            subject: z.string().describe("Message subject (supports {{variable}} placeholders)"),
            body: z.string().describe("Message body (supports {{variable}} placeholders)"),
            channels: z.array(z.enum(["email", "push", "sms"])).describe("Delivery channels"),
        },
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true },
    }, async (params) => {
        try {
            const res = await makeApiRequest("messages/templates", "POST", params);
