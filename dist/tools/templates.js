...
export function registerTemplateTools(server) {
    server.registerTool("dual_list_templates", {
        ...
        try {
            const res = await makeApiRequest("templates", "GET", undefined, params);
...
Example: Create a "Reward Token" template with properties like points, expiry_date, and redeemable status.`,
        ...
        try {
            const res = await makeApiRequest("templates", "POST", params);
    ...
    server.registerTool("dual_get_template", {
        ...
        try {
            const res = await makeApiRequest(`templates/${params.template_id}`);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_update_template", {
        ...
        try {
            ...
            const res = await makeApiRequest(`templates/${template_id}`, "PATCH", body);
    ...
    server.registerTool("dual_list_template_variations", {
        ...
        try {
            const res = await makeApiRequest(`templates/${params.template_id}/variations`);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_create_template_variation", {
        ...
        try {
            const { template_id, ...body } = params;
            const res = await makeApiRequest(`templates/${template_id}/variations`, "POST", body);
