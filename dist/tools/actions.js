...
The action_type must match a registered action type, and the object must belong to a template that allows it.`,
        ...
        try {
            const res = await makeApiRequest("ebus/actions", "POST", params);
...
Useful for complex operations like: mint + transfer + configure in one transaction.`,
        ...
        try {
            const res = await makeApiRequest("ebus/actions/batch", "POST", params);
    ...
    server.registerTool("dual_list_actions", {
        ...
        try {
            const res = await makeApiRequest("ebus/actions", "GET", undefined, params);
    ...
    server.registerTool("dual_get_action", {
        ...
        try {
            const res = await makeApiRequest(`ebus/actions/${params.action_id}`);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_list_action_types", {
        ...
        try {
            const res = await makeApiRequest("ebus/action-types", "GET", undefined, params);
            return textResult(JSON.stringify(res, null, 2));
        }
        catch (e) {
            return errorResult(handleApiError(e));
    ...
    server.registerTool("dual_create_action_type", {
        ...
        try {
            const res = await makeApiRequest("ebus/action-types", "POST", params);
    ...
    server.registerTool("dual_update_action_type", {
        ...
        try {
            ...
            const res = await makeApiRequest(`ebus/action-types/${action_type_id}`, "PUT", body);
