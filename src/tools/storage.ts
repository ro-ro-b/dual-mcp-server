...
export function registerStorageTools(server: McpServer): void {
  ...
  server.registerTool("dual_get_file", {
    ...
    try {
      const res = await makeApiRequest<Record<string, unknown>>(`storage/${params.file_id}`, "GET", undefined, { noRedirect: params.no_redirect });
  ...
  server.registerTool("dual_get_template_assets", {
    ...
    try {
      const res = await makeApiRequest<unknown>(`storage/template/${params.template_id}`);
      return textResult(JSON.stringify(res, null, 2));
    } catch (e) { return errorResult(handleApiError(e)); }
  });
}
